...
Original file is located at
    https://colab.research.google.com/drive/1Pp8vF2aaOGBOQ1eOTWaUrOYikz5F2pcg
...
def run_to_steady(F,
                  totalTime,
                  dt=0.01,
                  steady_rel_change=1e-3,   # tighter than 1e-2
                  steady_window=1.0,        # use 1 s window (≈ one period)
                  snapshot_times=None,
                  vel_damp=0.99,
                  vel_tol=1e-4,             # tip-velocity tolerance (m/s)
                  min_steady_time=0.5):     # don't stop before this time
    """
    Run dynamic relaxation starting from the reference helix under
    a constant tip load F (applied along -z at last node),
    until either totalTime is reached or the steady-state criterion is met.

    Returns:
        t_hist      : 1D array of times
        dz_hist     : 1D array of axial tip displacement delta_z(t)
        dz_star     : steady-state delta_z*
        snapshots   : list of (t, q) for requested snapshot_times
    """

    # --- Build external load for this run ---
    Fg = np.zeros(ndof)
    last = nv - 1
    ind_last = np.array([4*last, 4*last + 1, 4*last + 2])
    Fg[ind_last] = np.array([0.0, 0.0, -F])

    # --- Reset state to reference configuration ---
    qOld   = q_init.copy()
    uOld   = u_init.copy()
    a1_old = a1_init.copy()
    a2_old = a2_init.copy()
    ...
    dz_hist = np.zeros(Nsteps)

    endZ_0 = q_init[-1]   # initial z of last node
    ...
    step_window = max(1, int(round(steady_window / dt)))
    dz_star = None

    for step in range(Nsteps):

        # Implicit Euler step via objfun
        ...
        # simple velocity-proportional damping for dynamic relaxation
        u_new *= vel_damp
        ...
        # Snapshots at requested times
        if snapshot_times is not None and snap_idx < len(snapshot_times):
            if ctime >= snapshot_times[snap_idx] - 0.5*dt:
                snapshots.append((ctime, q_new.copy()))
                snap_idx += 1

        # ---------- Steady-state check ----------
        # Need at least 2 windows of history and some minimum time
        if step >= 2*step_window and ctime >= min_steady_time:
            dz_now   = dz_hist[step]
            dz_prev  = dz_hist[step - step_window]
            dz_prev2 = dz_hist[step - 2*step_window]

            denom_now  = max(abs(dz_now),  1e-12)
            denom_prev = max(abs(dz_prev), 1e-12)

            rel1 = abs(dz_now  - dz_prev)  / denom_now
            rel2 = abs(dz_prev - dz_prev2) / denom_prev

            v_tip = u_new[-1]   # z-velocity of the last node

            if (rel1 < steady_rel_change and
                rel2 < steady_rel_change and
                abs(v_tip) < vel_tol):

                dz_star = dz_now
                print(f"[run_to_steady] quasi-steady at t={ctime:.2f} s "
                      f"(rel1={rel1:.3e}, rel2={rel2:.3e}, v_tip={v_tip:.3e})")

                # trim histories and stop
                t_hist  = t_hist[:step+1]
                dz_hist = dz_hist[:step+1]
                break

        # Progress print
        if step % 50 == 0:
            print(f"[run_to_steady] step {step}/{Nsteps}, "
                  f"t={ctime:.2f} s, dz={dz_t:.4e} m")
    ...
    # If never triggered, take final value as "steady"
    if dz_star is None:
        dz_star = dz_hist[-1]

    return t_hist, dz_hist, dz_star, snapshots
...
# Helix parameters (match homework statement)
r0 = 0.001          # cross-sectional radius (d = 0.002 m)
d  = 2.0 * r0       # wire diameter
D  = 0.04           # mean coil diameter [m]
N  = 5              # number of turns
pitch = d           # pitch per turn p = d
...
# Characteristic bending force using contour length
F_char = EI / L**2
print(f"F_char = {F_char:.3e} N")
print(f"nv = {nv}, contour L = {L:.4f} m, axial L = {axial_l:.4f} m")

vectorLoad = np.array([0.0, 0.0, -F_char])  # default: use F_char
Fg = np.zeros(ndof)
c = nv - 1
ind = [4*c, 4*c + 1, 4*c + 2]
Fg[ind] += vectorLoad
...
# Save *reference* configuration to restart from for Parts (1)–(3)
q_init  = qOld.copy()
u_init  = uOld.copy()
a1_init = a1.copy()
a2_init = a2.copy()
...
def setup_helix_for_D(D_new, nv_new=50):
    """
    Rebuilds the global helix configuration for a given diameter D_new.
    Uses same d, N, material parameters, and nv.

    After this call, globals like L, axial_l, nodes, refLen, massMatrix,
    q_init, a1_init, etc. are updated, and run_to_steady() can be used.
    """
    global nv, ne, ndof
    global D, L, axial_l, nodes
    global EA, EI, GJ, dt, tol
    global massVector, massMatrix
    global refLen, voronoiRefLen
    global qOld, uOld, q_init, u_init
    global a1, a2, a1_init, a2_init
    global refTwist, kappaBar, twistBar
    global fixedIndex, freeIndex
...
# Initialize for the homework's base diameter D = 0.04 m
setup_helix_for_D(0.04, nv_new=50)
F_char = EI / L**2
print(f"Base D = 0.04 m, F_char = {F_char:.3e} N")

"""# Part 1"""

# =======================
# Part (1): Single Load F = F_char
# =======================

totalTime=20.0

# snapshot_times_part1 = [0.0, 1.0, 2.0, 3.0, 4.0]  # times for helix snapshots
snapshot_times_part1 = np.arange(0.0, totalTime + 1e-9, 1.0)

t1, dz1, dz1_star, snaps1 = run_to_steady(
    F_char,
    totalTime,
    dt=dt,
    steady_rel_change=0.01,   # 1% over 1 s
    steady_window=1.0,
    snapshot_times=snapshot_times_part1
)

print(f"Steady displacement at F = F_char: delta_z* = {dz1_star:.5e} m")

# Plot t vs delta_z(t)
plt.figure()
plt.plot(t1, dz1, 'o-', markersize=3)
plt.xlabel('t (s)')
plt.ylabel(r'$\delta_z(t)$ (m)')
plt.title('Part (1): Tip displacement vs time')
plt.grid(True)

# Mark steady-state value on the plot
plt.axhline(dz1_star, linestyle='--')
plt.show()

# Plot snapshots (you will grab 5 for the report)
for (tsnap, qsnap) in snaps1:
    plotrod_simple(qsnap, tsnap)

"""# Part 2"""

# =======================
# Part (2): Force sweep and linear fit
# =======================

F_min = 0.01 * F_char
F_max = 10.0 * F_char
nF    = 15   # number of sample force levels

# Log-spaced forces from 0.01 F_char to 10 F_char
F_values       = np.logspace(np.log10(F_min), np.log10(F_max), nF)
dz_star_values = np.zeros_like(F_values)

for i, F in enumerate(F_values):
    print(f"[Part 2] Running F = {F:.3e} N  ({i+1}/{nF})")

    # Use the same settings as in Part (1)
    _, _, dz_star_i, _ = run_to_steady(
        F,
        totalTime=totalTime,
        dt=dt,
        steady_rel_change=1e-3,   # or whatever you settled on for Part 1
        steady_window=1.0,
        snapshot_times=None,
        vel_damp=0.99
    )
    dz_star_values[i] = dz_star_i

# --------- Plot F vs delta_z* ----------
plt.figure()
plt.plot(F_values, dz_star_values, 'o-', label=r'$\delta_z^*(F)$')
plt.xlabel('F (N)')
plt.ylabel(r'$\delta_z^*$ (m)')
plt.title('Part (2): Force vs steady tip displacement')
plt.grid(True)

# --- linear fit in small-displacement region ---
# Use first ~1/3 of points (small forces → small displacements)
n_fit = max(3, nF // 3)
F_fit  = F_values[:n_fit]
dz_fit = dz_star_values[:n_fit]

# Fit F ≈ -k * delta_z*  (dz* is negative, k > 0)
A = (-dz_fit).reshape(-1, 1)      # design matrix
k_fit = np.linalg.lstsq(A, F_fit, rcond=None)[0][0]
print(f"Estimated linear stiffness k ≈ {k_fit:.4e} N/m")

# Plot the fitted line on the same axes
F_line  = np.linspace(0.0, F_fit.max(), 100)
dz_line = -F_line / k_fit         # from F = -k dz → dz = -F/k

plt.plot(F_line, dz_line, '--', label=f'Linear fit, k = {k_fit:.2e} N/m')
plt.legend()
plt.show()
...
# =======================
# Part (3): Diameter sweep vs textbook trend
# =======================
...
D_values = np.linspace(0.01, 0.05, 10)  # at least 10 values
k_values = np.zeros_like(D_values)

for i, D_val in enumerate(D_values):
    print(f"\n[Part 3] Setting up helix for D = {D_val:.3f} m")
    setup_helix_for_D(D_val, nv_new=50)

    # Recompute F_char for this D (using new L)
    F_char_D = EI / L**2

    # Force sweep for this D (like Part 2 but maybe fewer points)
    F_min = 0.01 * F_char_D
    F_max = 10.0 * F_char_D
    nF    = 8

    F_vals = np.logspace(np.log10(F_min), np.log10(F_max), nF)
    dz_star_vals = np.zeros_like(F_vals)

    for j, F in enumerate(F_vals):
        print(f"   D = {D_val:.3f} m, F = {F:.3e} N  ({j+1}/{nF})")
        _, _, dz_star_j, _ = run_to_steady(
            F,
            totalTime=20.0,
            dt=dt,
            steady_rel_change=0.01,
            steady_window=1.0,
            snapshot_times=None
        )
        dz_star_vals[j] = dz_star_j

    # Linear fit at small displacement to get k(D)
    n_fit = max(3, nF//3)
    F_fit  = F_vals[:n_fit]
    dz_fit = dz_star_vals[:n_fit]
    k_D = np.linalg.lstsq(dz_fit.reshape(-1,1), F_fit, rcond=None)[0][0]
    k_values[i] = k_D
    print(f"   Estimated k(D={D_val:.3f}) ≈ {k_D:.4e} N/m")

# Textbook stiffness parameter G d^4 / (8 N D^3)
x_text = G * d**4 / (8.0 * N * D_values**3)

plt.figure()
plt.plot(x_text, k_values, 'o', label='DER stiffness k(D)')
plt.plot(x_text, x_text, '--', label='Textbook line: k = G d^4/(8 N D^3)')
plt.xlabel(r'$G d^4 / (8 N D^3)$  (N/m)')
plt.ylabel(r'$k$ from DER (N/m)')
plt.title('Part (3): k vs textbook stiffness parameter')
plt.grid(True)
plt.legend()
plt.show()

plt.figure()
plt.plot(D_values, k_values, 'o-')
plt.xlabel('Helix diameter D (m)')
plt.ylabel('k(D) from DER (N/m)')
plt.title('Axial stiffness vs helix diameter')
plt.grid(True)
plt.show()


k_text = G * d**4 / (8.0 * N * D_values**3)
ratio  = k_values / k_text

plt.figure()
plt.plot(D_values, ratio, 'o-')
plt.xlabel('D (m)')
plt.ylabel(r'$k_\mathrm{DER} / k_\mathrm{text}$')
plt.title('Deviation from textbook stiffness vs diameter')
plt.grid(True)
plt.show()

plt.figure()
plt.plot(F_vals, dz_star_vals, 'o-', label=r'$\delta_z^*(F)$')
plt.plot(F_fit, dz_fit, 's', label='fit points')

F_line = np.linspace(0, F_fit.max(), 100)
dz_line = F_line / k_D
plt.plot(F_line, dz_line, '--', label='linear fit')

plt.xlabel('F (N)')
plt.ylabel(r'$\delta_z^*$ (m)')
plt.title(f'Force–displacement at D={D_val:.3f} m')
plt.grid(True)
plt.legend()
plt.show()
...
# =======================
# Re-plotting with positive stiffness
# =======================

# Flip sign so stiffness is positive like the textbook k
k_der_pos = -k_values          # or np.abs(k_values)

# Recompute textbook k(D) just to be explicit
k_text = G * d**4 / (8.0 * N * D_values**3)

# 1) k vs textbook parameter (now positive)
x_text = k_text   # x-axis is the textbook stiffness (same formula as before)

plt.figure()
plt.plot(x_text, k_der_pos, 'o', label='DER stiffness k(D) (|k|)')
plt.plot(x_text, x_text, '--', label='Textbook line: k = G d^4/(8 N D^3)')
plt.xlabel(r'$G d^4 / (8 N D^3)$  (N/m)')
plt.ylabel(r'$k$ from DER (N/m)')
plt.title('Part (3): k vs textbook stiffness parameter (sign-corrected)')
plt.grid(True)
plt.legend()
plt.show()

# 2) Axial stiffness vs helix diameter (sign-corrected)
plt.figure()
plt.plot(D_values, k_der_pos, 'o-')
plt.xlabel('Helix diameter D (m)')
plt.ylabel(r'$k(D)$ from DER (N/m)')
plt.title('Axial stiffness vs helix diameter (|k|)')
plt.grid(True)
plt.show()

# 3) Ratio of DER to textbook stiffness (using magnitudes)
ratio_mag = k_der_pos / k_text

plt.figure()
plt.plot(D_values, ratio_mag, 'o-')
plt.xlabel('D (m)')
plt.ylabel(r'$|k_\mathrm{DER}| / k_\mathrm{text}$')
plt.title('Deviation from textbook stiffness vs diameter (magnitudes)')
plt.grid(True)
plt.show()