...
def plot(x, index_matrix, t):
    ...
    for i in range(index_matrix.shape[0]):
        ...
        xj = x[ind[2]]
        yj = x[ind[3]]
        plt.plot([xi, xj], [yi, yj], 'bo-')  # Blue line with circles at the nodes
...
plot_times = [0, 0.1, 1, 10, 100]

# free indices
free_DOF = np.array([2, 3, 6, 7])


# Container to store y-coordinate of node 1 & 3
y_node1 = np.zeros(len(t))  # Node 1 (y1 = x[3])
y_node3 = np.zeros(len(t))  # Node 3 (y3 = x[7])
y_node1[0] = x_old[3]
y_node3[0] = x_old[7]

# Container to store x-coordinate of node 1 & 3
x_node1 = np.zeros(len(t))
x_node3 = np.zeros(len(t))
x_node1[0] = x_old[2]
x_node3[0] = x_old[6]

# Plot initial configuration at t = 0
...
for k in range(len(t)-1):
  ...
  if np.isclose(t_new, plot_times, atol=1e-5).any():
...
# Plot Y Position of Free Nodes vs. Time
plt.figure(figsize=(8, 5))
plt.plot(t, y_node1, 'ro-', label='Node 1 (red)')
plt.plot(t, y_node3, 'bo-', label='Node 3 (blue)')
plt.xlabel('Time (s)')
plt.ylabel('Y Position (m)')
plt.title('Y Position of Free Nodes vs. Time')
plt.xlim(0, 10)  # Limit x-axis to 0–10 seconds
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.figure(figsize=(8, 5))
plt.plot(t, x_node1, 'ro-', label='Node 1 (red)')
plt.plot(t, x_node3, 'bo-', label='Node 3 (blue)')
plt.xlabel('Time (s)')
plt.ylabel('X Position (m)')
plt.title('X Position of Free Nodes vs. Time')
plt.xlim(0, 10)
plt.legend()
plt.grid(True)
plt.tight_layout()
